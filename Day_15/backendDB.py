...
class myDatabase:
    def __init__(self,dbname):
        ...
        self.cur.execute("Create Table if not exists Employee (Id integer primary key autoincrement, Name varchar (50), Email Varchar (50), DOB Datetime)")
        self.conn.commit()
    ...
    def updateData(self,Id,Name,Email,DOB):
        self.cur.execute("Update Employee Set Name=?, Email=?, DOB=? where Id=? ", (Name,Email,DOB,Id))
        self.conn.commit()
    ...
    def Delete(self):
        ...
        if self.searchData(idnum):
            ...
            if yn=="y" or yn=="Y":
                self.cur.execute("Delete from Employee where Id=? ", (idnum,))
                self.conn.commit()
